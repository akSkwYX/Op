...
exception Lexing_error of string * int
...
(* ----- Several characters words ----- *)

let string s current line =
  let rec aux value current' line' =
    if current' < 0 then
      raise (Lexing_error ("Unfinished string", line'))
    else
...
let path s current line =
  let rec aux value current' line' =
    if current' < 0 then
      raise (Lexing_error ("Unfinished path", line'))
    else
      match s.[current'] with
      | '\'' -> value, (current'-1), line'
      | '\n' -> aux ("\n"^value) (current'-1) (line'+1)
...
let number s current line =
  let rec aux value current' line' is_float =
    match s.[current'] with
    | '.' when is_float -> raise ( Lexing_error ("Numbers are declared as <int>.<int> for floats or <int> for integers", line') )
...
let tokenize (s:string) =
  ...
  let rec aux res current line =
    ...
    | '~' -> let rec skip_until current' line' =
               if current' < 0 then
                 raise (Lexing_error ("Unclosed comment", line))
               else
                 match s.[current'] with
    ...
    | '&' ->
      ...
      | _ -> raise (Lexing_error ("Unknow character : '&'", line))
      end
    | '|' ->
      begin
      match s.[current - 1] with
    ...
    | c -> 
      if isDigit(c) then
        ...
        raise (Lexing_error ("Unexpected char : " ^ String.make 1 s.[ current ], line))
  in
  try aux [] (len-1) 0 with
    | Lexing_error (err, line) -> 
        print_endline ("[ " ^ string_of_int (Utility.number_of_line s - line) ^ " ] " ^ err); []
